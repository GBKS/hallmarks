...
#
#   _gen_pixels(cells)         → bytearray(280), one byte per pixel
#   _gen_pixels_packed(cells)  → bytearray(70),  four pixels per byte
#
# Each public entry point calls the generator that matches its return type
# directly, so neither path pays a pack/unpack penalty.
#
# Packed layout: 2 bits per pixel, MSB-first.
#
...
# The packed form costs 70 bytes vs 280 — useful on devices that keep several
# grids in RAM simultaneously.
# =============================================================================

def _gen_pixels(cells):
    """Generate the 14x20 pixel grid as a flat bytearray (280 bytes).

    Returns bytearray(280), row-major, values 0/1/2.
    """
    px = bytearray(280)
    for y in range(7):
        for x in range(5):
            ...
            bx = x * 3
            by = y * 3
            if v == 1:              # primary: solid 2x2 block
                px[by * 14 + bx]           = 1
                px[by * 14 + bx + 1]       = 1
                px[(by + 1) * 14 + bx]     = 1
                px[(by + 1) * 14 + bx + 1] = 1
